...
run "$GCLOUD" compute networks subnets create "$PSC_SUBNET" \
...
run "$GCLOUD" compute backend-services create "$BACKEND_SERVICE" \
...
run "$GCLOUD" compute addresses create "${PREFIX}-ilb-ip" \
...
run "$GCLOUD" compute url-maps create "${PREFIX}-urlmap" \
  --default-service="$BACKEND_SERVICE" --region="$REGION" --project="$PROJECT_ID"
run "$GCLOUD" compute target-http-proxies create "${PREFIX}-proxy" \
  --url-map="${PREFIX}-urlmap" --region="$REGION" --project="$PROJECT_ID"
run "$GCLOUD" compute forwarding-rules create "${PREFIX}-fr" \
...
# service must be a fully-qualified backend service reference; authority is then
# required and carries the internal FQDN. failOpen:false is the ADR's posture.
...
service: https://www.googleapis.com/compute/v1/projects/${PROJECT_ID}/regions/${REGION}/backendServices/${BACKEND_SERVICE}
authority: ${EXT_FQDN}
failOpen: false
timeout: ${EXT_TIMEOUT}
